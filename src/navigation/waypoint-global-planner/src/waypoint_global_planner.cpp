...
void WaypointGlobalPlanner::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
{
  if(!initialized_)
  {
    ...
    waypoint_sub_ = pnh.subscribe("/path_odom", 100, &WaypointGlobalPlanner::waypointCallback, this);
...
void WaypointGlobalPlanner::waypointCallback(const nav_msgs::Odometry & odomgoal)
{
  if (clear_waypoints_)
  {
    waypoints_.clear();
  ...
  waypoints_.back().header.stamp = odomgoal.header.stamp;
  waypoints_.back().header.frame_id = "map";
  waypoints_.back().pose.position.x = odomgoal.pose.pose.position.x;
  waypoints_.back().pose.position.y = odomgoal.pose.pose.position.y;
  ...
  //{
    ...
    path_.header.stamp = odomgoal.header.stamp;
