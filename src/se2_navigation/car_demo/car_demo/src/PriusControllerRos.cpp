...
void PriusControllerRos::loadPIDParameters() {
  const std::string controllerParametersFilename = nh_->param<std::string>("/prius_pid_parameters_filename", "");
...
void PriusControllerRos::createPathTrackerAndLoadParameters() {
  const std::string controllerParametersFilename = nh_->param<std::string>("/prius_path_tracker_ros_parameters_filename", "");
...
void PriusControllerRos::translateGear(double longitudinalSpeed, prius_msgs::PriusControl* ctrl) const {
...
bool PriusControllerRos::currentStateRequestService(CurrentStateService::Request& req, CurrentStateService::Response& res) {
  res.pose = priusState_.pose.pose;
  res.twist = priusState_.twist.twist;

  return true;
}
bool PriusControllerRos::controllerCommandService(ControllerCommandService::Request& req, ControllerCommandService::Response& res) {
...
void PriusControllerRos::processStartTrackingCommand() {
  if (!planReceived_) {
    ROS_WARN_STREAM("PriusControllerRos:: Rejecting  the start command since the robot hasn't received a plan yet");
    return;
  }

  if (currentlyExecutingPlan_) {
    ROS_WARN_STREAM("PriusControllerRos:: Rejecting  the start command since the robot is already executing another plan");
    return;
  }
